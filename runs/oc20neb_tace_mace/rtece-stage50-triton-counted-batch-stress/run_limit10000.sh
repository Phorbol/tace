#!/bin/bash
#SBATCH --job-name=rtece-counted-10k
...
#SBATCH --output=/home/gengjianrui/bin/logs/rtece-counted-10k-%j.out
#SBATCH --error=/home/gengjianrui/bin/logs/rtece-counted-10k-%j.err
...
run_one() {
  local backend=$1
  local label=$2
  local output="${OUT}/radial8h24_direct_active_${label}_limit10000_invalid_update_only.json"
  local status="${OUT}/radial8h24_direct_active_${label}_limit10000_invalid_update_only.status"
...
run_one torch_radius_nopbc_triton_padded triton_padded
run_one torch_radius_nopbc_triton_counted triton_counted
