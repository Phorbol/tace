#!/bin/bash
#SBATCH --job-name=rtece-provider-meta
...
#SBATCH --output=/home/gengjianrui/bin/logs/rtece-provider-meta-%j.out
#SBATCH --error=/home/gengjianrui/bin/logs/rtece-provider-meta-%j.err
...
OUT=runs/oc20neb_tace_mace/rtece-stage46-provider-metadata
...
for chunk in 64 128 256 512; do
  run_backend torch_radius_nopbc_grouped_chunked chunk${chunk} --graph-update-chunk-configs "${chunk}"
done
