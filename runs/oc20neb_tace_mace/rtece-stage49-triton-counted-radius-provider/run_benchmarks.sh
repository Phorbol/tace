#!/bin/bash
#SBATCH --job-name=rtece-counted-radius
...
#SBATCH --output=/home/gengjianrui/bin/logs/rtece-counted-radius-%j.out
#SBATCH --error=/home/gengjianrui/bin/logs/rtece-counted-radius-%j.err
...
OUT=runs/oc20neb_tace_mace/rtece-stage49-triton-counted-radius-provider
...
run_backend() {
  ...
  "${PY}" benchmarks/oc20neb_tace_mace/benchmark_rtece_scalar.py \
    ...
    --output "${OUT}/radial8h24_direct_active_${label}_invalid_update_only.json"

  "${PY}" benchmarks/oc20neb_tace_mace/benchmark_rtece_scalar.py \
    "${COMMON[@]}" \
    --graph-update-backend "${backend}" \
    "$@" \
    --output "${OUT}/radial8h24_direct_active_${label}_invalid_model_updates.json"
}
...
run_backend torch_radius_nopbc_triton_counted triton_counted
