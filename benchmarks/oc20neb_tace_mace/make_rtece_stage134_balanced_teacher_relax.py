#!/usr/bin/env python3
"""Create Stage134 balanced teacher-relax distillation wrappers."""
...
STAGE134_VARIANTS = STAGE133_VARIANTS
...
def _stage134_rows() -> list[dict[str, Any]]:
    ...
    rows = []
    for name in STAGE134_VARIANTS:
        row = dict(by_name[name])
        row["stage_basis"] = "stage134_balanced_teacher_relax_distill"
        row["distillation_test_role"] = (
            "fixed stage133 atomic backbone; only DFT-anchor/teacher-trajectory sample weights change"
        )
        row["stage134_source_basis"] = (
            "Stage133 showed teacher-relax trajectory labels slightly improve F RMSE and rattle force tail but cause "
            "large energy drift and fail benchmark/physical gates. Stage134 keeps architecture and trajectory data fixed "
            "while balancing source weights: base DFT remains the energy anchor and teacher trajectories contribute mostly "
            "force/Sobolev information."
        )
        axes = list(row.get("tece_axes") or [])
        for axis in (
            "stage134_balanced_teacher_relax_distillation",
            "dft_energy_anchor",
            "teacher_relax_force_sobolev_weighting",
...
def make_stage134_manifest(
    *,
    ...
    teacher_energy_multiplier: float = 0.25,
    ...
    for name, value in (
        ("base_energy_multiplier", base_energy_multiplier),
        ("teacher_energy_multiplier", teacher_energy_multiplier),
        ("teacher_force_multiplier", teacher_force_multiplier),
    ):
        if float(value) < 0.0:
    ...
    weighted_name = f"weighted_train_base{int(base_limit_configs)}_plus_teacher_relax{trajectory_frame_count}_eanchor.extxyz"
    artifacts = {
        ...
        "prep_wrapper": str(root / "rtece_stage134_prepare_no_export.sbatch"),
        "wrapper_root": str(root / "balanced_teacher_relax_wrappers"),
        "run_root": str(root / "balanced_teacher_relax_runs"),
    ...
    weight_policy = {
        "schema_version": "rtece_stage134_source_balance.v1",
        "source_energy_multipliers": {base_label: float(base_energy_multiplier), teacher_label: float(teacher_energy_multiplier)},
        "source_force_multipliers": {teacher_label: float(teacher_force_multiplier)},
        "normalize_energy_mean": True,
        "normalize_force_mean": True,
        "purpose": "Keep DFT base structures as the energy zero/PES anchor while using teacher relaxation trajectories mainly as force-Sobolev deployment constraints.",
    }
    steps = [
        {
        ...
        {
            "id": "apply_dft_anchor_teacher_relax_weights",
            "command": _command([
                py,
                ...
                "--source-energy-multiplier", f"{teacher_label}:{float(teacher_energy_multiplier)}",
    ...
    return {
        "schema_version": "rtece_stage134_balanced_teacher_relax_distill.v1",
        "stage": "stage134_balanced_teacher_relax_distill",
        "row_set": "stage134-balanced-teacher-relax-distill",
        "distillation_semantics": "dft_energy_anchor_plus_teacher_relax_force_weighted_distillation",
        "comparison_question": (
            "Does source-balanced teacher-relax distillation repair the stage133 energy drift while preserving force-tail "
            "benefits, thereby testing distillation-measure error before blaming projection error or changing architecture?"
        ),
        ...
        "rows": [{"variant": str(row["name"]), **row} for row in _stage134_rows()],
        "artifacts": artifacts,
        "steps": steps,
        "review_basis": [
...
def audit_stage134_manifest(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        "schema_version": payload.get("schema_version") == "rtece_stage134_balanced_teacher_relax_distill.v1",
        "stage": payload.get("stage") == "stage134_balanced_teacher_relax_distill",
        "distillation_semantics": payload.get("distillation_semantics") == "dft_energy_anchor_plus_teacher_relax_force_weighted_distillation",
        ...
        "weighted_train_artifact": "weighted_train" in (payload.get("artifacts") or {}),
        "single_fixed_atomic_row": variants == list(STAGE134_VARIANTS),
        "uses_teacher_relax_generator": "make_teacher_relax_distill_configs.py" in commands,
        "uses_source_weight_tool": "apply_extxyz_sample_weights.py" in commands and "--source-energy-multiplier" in commands,
        "normalizes_energy_weights": bool(policy.get("normalize_energy_mean")) and "--normalize-energy-mean" in commands,
        "normalizes_force_weights": bool(policy.get("normalize_force_mean")),
    ...
    return {
        "schema_version": "rtece_stage134_manifest_audit.v1",
...
def format_audit_markdown(audit: dict[str, Any]) -> str:
    lines = [
        "# Stage134 Manifest Contract Audit",
        "",
...
def write_prepare_wrapper(path: str | Path, payload: dict[str, Any]) -> Path:
    ...
    body = [
        ...
        "#SBATCH --output=/home/gengjianrui/bin/logs/rtece-prep134-%j.out",
        "#SBATCH --error=/home/gengjianrui/bin/logs/rtece-prep134-%j.err",
        "",
        "set -euo pipefail",
        "set -x",
...
def _stage134_physical_cases(payload: dict[str, Any]) -> list[dict[str, Any]]:
    cases = []
        ...
        cases.append(
            {
                ...
                "selection_basis": "stage134 DFT-anchor teacher-relax source-weight balance ablation",
            }
        )
    return cases


def materialize_stage134(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    physical = write_physical_triage_wrappers(
        ...
        cases=_stage134_physical_cases(payload),
        stage="stage134_physical_triage",
        source="runs/oc20neb_tace_mace/rtece-stage134-balanced-teacher-relax/stage134_interpretation.md",
        design_basis=(
            "Evaluate whether DFT energy anchoring plus teacher-relax force weighting repairs stage133 energy drift "
            "without giving up trajectory force-tail benefits."
        ),
        job_name="rtece-phys134",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--teacher-energy-multiplier", type=float, default=0.25)
...
def main() -> None:
    ...
    payload = make_stage134_manifest(
        ...
        teacher_energy_multiplier=args.teacher_energy_multiplier,
    ...
    if args.materialize:
        payload = materialize_stage134(payload)
