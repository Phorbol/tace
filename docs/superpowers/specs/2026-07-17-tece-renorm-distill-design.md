...
- The next clean backend target is a PBC-aware batched cell-list/neighbor provider, ideally one batched kernel over the whole graph window rather than one Python loop per config. This keeps the TECE/TACE route theoretically clean: semantic scalar projection and fused force kernels are already validated; the remaining deployment axis is safe, fast edge-topology refresh.
